...
export function SummaryCard() {
  ...
  const income = transactions.reduce(
    (acc, transaction) =>
      transaction.type === 'income' ? acc + transaction.price : acc,
    0,
  )

  return (
    <div
      className={css({ background: '#323238', borderRadius: '6px', p: '2rem' })}
    >
      <header
        className={css({
          display: 'flex',
      ...
      <strong
        className={css({ display: 'block', mt: '1rem', fontSize: '2rem' })}
      >
...
export function SummaryCardLeave() {
  ...
  const outcome = transactions.reduce(
    (acc, transaction) =>
      transaction.type === 'outcome' ? acc + transaction.price : acc,
    0,
  )
...
export const SummaryCardTotal: React.FC<SummaryCardProps> = ({ variant }) => {
  ...
  return (
    <div
      className={css({
        background: variant === 'green' ? '#015F43' : '#323238',
        borderRadius: '6px',
        p: '2rem',
      })}
    >
